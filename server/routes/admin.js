...
router.post('/testEvent', async(req, res)=>{
    let uid = "admin"
    let totalAmount = 1300000
    let orderName = "admin"
    try{
        ...
        if(!findVip){
            ...
            if(totalTargetPrice>=1000000){
                ...
                let targetPrice = 1000000 - totalTargetPrice
                console.log("Vip가 되려면 "+targetPrice+"원 남았습니다.")
            }
            res.status(200).json()
        }else{
            console.log("Vip")
            ...
            if(totalTargetPrice>=1500000){
                ...
                vip.userName = orderName;
                vip.status = 1+1
                let vipsave = await vip.findOneAndUpdate({"uid":uid},{$set:{ttl:5000}});
            }
            else{
                let targetPrice = 1000000 - totalTargetPrice
                console.log("Vip가 되려면 "+targetPrice+"원 남았습니다.")
